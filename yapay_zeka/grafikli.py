...
# Özel Callback sınıfı (her epoch sonunda log kaydetme)
class CustomCallback(Callback):
    def __init__(self, log_file):
        super().__init__()
        self.log_file = log_file

        # Dosyayı temizlemek veya başlangıç mesajı eklemek için kullanılır
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # Epoch bilgilerini konsola yazdır
        print(
...
def main(train_model=True):
    if train_model:
        ...
        # Eğitim sırasında logları kaydetmek için CustomCallback kullan
        model.fit(
            X_train_pad,
            np.array(y_train),
            epochs=5,
...
if __name__ == "__main__":
    main(train_model=True)  # Eğitim yapmadan tahmin yapmak için False olarak değiştir!
